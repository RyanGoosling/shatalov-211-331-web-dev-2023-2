...
def unique_network_map(topology_dict):
    ...
    for value in topology_dict.values():
        twice = unique_topology.get(value, False)
        if twice:
            del unique_topology[twice]
            
    return unique_topology

def compile_topology():
    full_topology = unique_network_map(task_11_2.create_network_map(infiles))
    draw_network_graph.draw_topology(full_topology)
